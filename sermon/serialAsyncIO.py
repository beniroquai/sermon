...
class Serial:
    ...
    def extract_json_objects(self, s):
        ...
        dictionaries = []
        last_position = 0
        #self._logger.debug(f"Extracting JSON objects from: {s}")
        remainder = ""
        for part in parts:
...
async def main():
    ...
    while True:
        ...
        messages = [message, message3]
        
        for message in messages:
            await mSerial.sendMessage(data=message, blocking=True, mTimeout=2)
